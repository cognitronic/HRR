...
namespace HRR.Website
{
    public partial class CommentProperties : HRRBasePage
    {
        ...
        private void SaveResponse()
        {
            ...
            string emails = "";
            foreach (var r in CurrentComment.Communication)
            {
            ...
            sb.Append("<h2><span>Comment Response Activity</span></h2>");
            sb.Append("<div class='maincontent'>");
            sb.Append("Click here to view: <a href='");
            sb.Append(ConfigurationManager.AppSettings["BASEURL"]);
            sb.Append("/Comments/");
            sb.Append(c.CommentID.ToString());
            sb.Append("'>HRRiver.com</a></div></div>");
            sb.Append(EmailHelper.EmailHTMLEnd());
            try
            {
                IdeaSeed.Core.Mail.EmailUtils.SendEmail(CurrentComment.EnteredForRef.Email, "noreply@hrrriver.com", "", emails + ConfigurationManager.AppSettings["NEWGOALNOTIFICATION"], "New Comment Response Activity", sb.ToString(), false, "");
            }
            catch (Exception exc)
            {

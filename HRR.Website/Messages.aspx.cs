...
namespace HRR.Website
{
    public partial class Messages : HRRBasePage
    {
        ...
        protected void CheckAllSelectedIndexChanged(object o, EventArgs e)
        {
            foreach (DataListItem item in dlMessages.Items)
            {
                var cbone = item.FindControl("cbSelected") as IdeaSeed.Web.UI.CheckBox;
                if (cbone.Checked)
                    cbone.Checked = false;
                else
                    cbone.Checked = true;
                        ...
                        if (cb.Checked)
                        {
                            ...
                            foreach (var s in new NotificationSubscriberServices().GetByNotificationIDAccountID((int)Notification.REPORT_MESSAGE_TO_HR, SecurityContextManager.Current.CurrentAccount.ID))
                            {
                                if (m.RecipientID != s.PersonID)
                                {
                                    ...
                                    IdeaSeed.Core.Mail.EmailUtils.SendEmail(s.Subscriber.Email, "flagged@hrriver.com", "Message Flagged For Review", sb.ToString());
                                }
                            }
                        }
                    }
        ...
        protected void MessagesItemDataBound(object o, DataListItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (r != null)
                {
                    ...
                    if (r.MessageStatusTypeID == (int)MessageStatusType.REPORT_TO_HR)
                    {
                        var flagged = e.Item.FindControl("divContainer") as HtmlControl;
                        flagged.Attributes["class"] = "flaggedmessage";
                    }
                }
            }
        }

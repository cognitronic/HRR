...
namespace HRR.Website
{
    public partial class Goal : HRRBasePage
    {
        ...
        IList<HRR.Website.Views.GoalMilestoneView> MilestoneControls
        {
            get
            {
                if (SessionManager.Current["milestone_controls"] != null)
                    return (IList<HRR.Website.Views.GoalMilestoneView>)SessionManager.Current["milestone_controls"];
                else
                    SessionManager.Current["milestone_controls"] = new List<HRR.Website.Views.GoalMilestoneView>();
                return (IList<HRR.Website.Views.GoalMilestoneView>)SessionManager.Current["milestone_controls"];
            }
            set
            {
        ...
        protected void CreateUserControl(string controlID)
        {
            try
            {
                ...
                {
                    if (createAgain && divMilestoneContainer != null)
                    {
                ...
                else
                {
                    if (createAgain && divMilestoneContainer != null)
                    {
                        if (MilestoneControls.Count > 0)
                        {
                            ...
                            {
                                HRR.Website.Views.GoalMilestoneView foc = new HRR.Website.Views.GoalMilestoneView();
                                foc = Page.LoadControl("~/Views/GoalMilestoneView.ascx") as HRR.Website.Views.GoalMilestoneView;
                                foc.ID = c.ID;
                                divMilestoneContainer.Controls.Add(foc);
                            }
                            foreach (var ms in CurrentGoal.Milestones.OrderBy(o => o.DueDate))
                            {
                                Control c = LoadControl("/Views/GoalMilestoneView.ascx");
        ...
        public void LoadTabRecordCount()
        {
            if (CurrentGoal.Managers.Count > 0)
            {
                ...
                rtsGoal.Tabs.FindTabByValue("2").Text += "subscribers (" + CurrentGoal.Managers.Count.ToString() + ")";
            }
            if (CurrentGoal.Milestones.Count > 0)
            {
                UpdateProgressIndicator();
                rtsGoal.Tabs.FindTabByValue("1").Text = "";
                rtsGoal.Tabs.FindTabByValue("1").Text += "milestones (" + CurrentGoal.Milestones.Count.ToString() + ")";
            }

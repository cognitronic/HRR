    ...
    public partial class UpcomingEventsListView : System.Web.UI.UserControl
    {
        ...
        protected void ItemDataBound(object o, DataListItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                {
                    lbl.Text = "<span style='font-size: 10pt;'><a href='/People/" + item.Email + "'>" + item.Name + "'s</a></span> birthday is <span style='color: #ff6600;'>" + ((DateTime)item.Birthdate).Month.ToString() + "/" + ((DateTime)item.Birthdate).Day.ToString() + "</span>";
                }
                else
                {
                    lbl.Text = "<span style='font-size: 10pt;'><a href='/People/" + item.Email + "'>" + item.Name + "'s</a></span> anniversary is <span style='color: #ff6600;'>" + ((DateTime)item.HireDate).Month.ToString() + "/" + ((DateTime)item.HireDate).Day.ToString() + "</span>";
                }
